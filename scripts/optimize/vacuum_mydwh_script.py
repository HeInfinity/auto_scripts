    ...
    for line in log_content:
        ...
        try:
            ...
            if log_time >= start_time:
                ...
                if "所有表的VACUUM ANALYZE操作完成, 总计耗时" in line:
                    total_times.append(cleaned_line)
                elif "错误" in line:
                    errors.append(cleaned_line)
                elif "完成表" in line and "的VACUUM ANALYZE操作" in line:
                    vacuum_times.append(cleaned_line)
...
def format_email_body(log_info: Dict[str, List[str]], success: bool) -> str:
    ...
    if log_info['vacuum_times']:
        # 保持原始日志格式
        formatted_vacuum = [f"{current_time} INFO {line}" for line in log_info['vacuum_times']]
        log_sections.append('\n'.join(formatted_vacuum))
