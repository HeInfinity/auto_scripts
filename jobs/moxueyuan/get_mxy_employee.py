        ...
        try:
            async with conn.cursor() as cursor:
                ...
                if result and result['token']:
                    token = result['token']
                    logger.info(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [token] [INFO] 成功获取access_token: {token}")
                    return token
                else:
                    logger.error(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [token] [ERROR] 数据库中未找到access_token")
                    return None
        finally:
            await db_manager.release_connection('myDB_Alicloud', conn)
    except Exception as e:
        logger.error(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [token] [ERROR] 获取access_token时发生错误: {str(e)}")
        return None
    ...
    while True:
        ...
        try:
            ...
            if data['status'] == 'Y' and 'results' in data:
                ...
                logger.info(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [exam_{exam_id}] [INFO] 获取考试数据: {exam_name}-{status_description}, 第{page}页")
                
                page_data = pd.DataFrame(data['results'])
                selected_columns = ['examId', 'userid', 'reexam', 'userName', 'makeUp', 
                                  'stateValue', 'state', 'examName', 'gradetime']
                page_data = page_data[selected_columns]
                ...
                logger.error(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [exam_{exam_id}] [ERROR] 获取数据失败: {exam_name}-{status_description}, 第{page}页")
                break
        except requests.exceptions.RequestException as e:
            logger.error(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [exam_{exam_id}] [ERROR] 请求发生错误: {str(e)}")
            # 如果是最后一次重试也失败了，就退出循环
            if getattr(e.response, 'status_code', None) in (429, 403):  # 如果是频率限制，多等待一会
                logger.warning(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [exam_{exam_id}] [WARN] 检测到请求频率限制，等待60秒后继续...")
                time.sleep(60)
                continue
            break
    ...
    while True:
        ...
        try:
            ...
            if data['status'] == 'Y' and 'results' in data:
                ...
                logger.info(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [course_{course_id}] [INFO] 获取课程数据: {course_name}, 第{page}页")
                ...
                logger.error(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [course_{course_id}] [ERROR] 获取课程数据失败: {course_name}, 第{page}页")
                break
        except requests.exceptions.RequestException as e:
            logger.error(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [course_{course_id}] [ERROR] 请求发生错误: {str(e)}")
            # 如果是最后一次重试也失败了，就退出循环
            if getattr(e.response, 'status_code', None) in (429, 403):  # 如果是频率限制，多等待一会
                logger.warning(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [course_{course_id}] [WARN] 检测到请求频率限制，等待60秒后继续...")
                time.sleep(60)
                continue
            break
    ...
    try:
        ...
        try:
            async with conn.cursor() as cursor:
                ...
                logger.info(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [db] [INFO] 已清空表 {table_name}")
                ...
                for i in range(0, len(df), batch_size):
                    ...
                    logger.info(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [db] [INFO] 已插入 {min(i + batch_size, len(df))}/{len(df)} 条记录到表 {table_name}")
        finally:
            await db_manager.release_connection('myDB_Alicloud', conn)
    except Exception as e:
        logger.error(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [db] [ERROR] 保存数据到数据库时发生错误: {str(e)}")
        raise

async def process_and_save_data():
    """主处理函数"""
    try:
        logger.info(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [main] [INFO] 开始执行数据获取任务")
        db_manager = DBManager(logger=logger)
        
        # 获取access_token
        access_token = await get_access_token(db_manager)
        if not access_token:
            logger.error(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [main] [ERROR] 无法获取access_token，任务终止")
            return
            
        # 处理每个考试
        for exam_name, exam_id in EXAM_CONFIG.items():
            logger.info(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [exam_{exam_id}] [INFO] 开始处理考试: {exam_name}")
            ...
            # 获取课程数据
            course_data = None
            if '3964706' in COURSE_CONFIG.values():  # 只获取特定课程数据
                course_data = fetch_all_course_data(access_token, '3964706')
            
            # 合并数据
            if course_data is not None and not course_data.empty:
                final_data = pd.merge(combined_data, course_data, on='userid', how='left')
            else:
        ...
        logger.error(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [main] [ERROR] 处理数据时发生错误: {str(e)}")
    finally:
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [main] [ERROR] 程序执行出错: {str(e)}")